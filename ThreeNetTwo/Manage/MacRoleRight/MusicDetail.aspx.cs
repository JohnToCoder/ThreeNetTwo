...
namespace ThreeNetTwo.Manage.MacRoleRight
{
    public partial class MusicDetail : System.Web.UI.Page
    {
        ...
        private void Bind(string strID, string strMenuTypeID, string strRoleID)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                GvMusicDetail.DataSource = dt;
                GvMusicDetail.DataBind();

                for (int i = 0, rowCount = GvMusicDetail.Rows.Count; i < rowCount; i++)
                {
                    GvMusicDetail.Rows[i].Attributes.Add("onmouseover", "c=this.style.backgroundColor;this.style.backgroundColor='#cdeaf2'");
                    GvMusicDetail.Rows[i].Attributes.Add("onmouseout", "this.style.backgroundColor=c");
                }
            }
            else
            {
        ...
        private void getNullValue(DataTable dt)
        {
            ...
            GvMusicDetail.DataSource = dt;
            GvMusicDetail.DataBind();
            GvMusicDetail.Rows[0].Cells.Clear();
            GvMusicDetail.Rows[0].Cells.Add(new TableCell());
            GvMusicDetail.Rows[0].Cells[0].ColumnSpan = dt.Columns.Count;
            GvMusicDetail.Rows[0].Cells[0].Text = "<font color='red'>None!</font>";
            GvMusicDetail.Rows[0].Cells[0].Style.Add("text-align", "center");
            GvMusicDetail.Rows[0].Cells[0].Style.Add("border", "solid 1px #567ab2");
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void GvMusicDetail_RowDataBound(object sender, GridViewRowEventArgs e)
        {
        ...
        /// <summary>
        /// 開發功能：GvMusicDetail翻頁功能
        /// 開發人員：楊碧清
        /// 開發時間：2011-04-12
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void GvMusicDetail_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            DataTable dtbs = ViewState["dt"] as DataTable;

            GvMusicDetail.PageIndex = e.NewPageIndex;

            GvMusicDetail.DataSource = dtbs;
            GvMusicDetail.DataBind();
